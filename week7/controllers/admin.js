...
async function postCourse (req, res, next) {
  ...
  if (isNotValidUuid(id) || isNotValidUuid(skillId)) {
...
async function putCoachProfile (req, res, next) {
  ...
  if (
    skillIds.length === 0 ||
    skillIds.every((skill) => isUndefined(skill) || isNotValidUuid(skill))
  ) {
    logger.warn('欄位未填寫正確，skill id 格式不正確');
    next(appError(400, '欄位未填寫正確'));
    return;
  }
  const coachRepo = dataSource.getRepository('Coach');
